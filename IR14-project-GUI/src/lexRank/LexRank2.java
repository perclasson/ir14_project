...
public class LexRank2 {
		...
		for (int i = 0; i < names.length; i++) {
			...
			for (String word : words) {
				if (map.containsKey(word))
					...
					// första gången ett ord dyker upp i ett dokument, räkna upp
					// df för ordet
	...
	/**
	 * calculate: sqrt( sum w in s (tf_w,s * idf_w)^2 ) for each sentence tf_w,s
	 * = term freq of word w in sentence s idf_w = inverted document freq of
	 * word w
	 */
	...
	private void similarity() {
		...
		for (int i = 0; i < names.length; i++) {
			...
			for (int j = i + 1; j < names.length; j++) {
				...
				// Loopa igenom alla ord i ena meningen och kolla om de även
				// finns i andra
				// Om de finns, addera produkten av deras tf och idf till
				// summan.
	...
	private void sortSentRanks(double[] ranks) {
		sentRanks = new Sentence[ranks.length];
